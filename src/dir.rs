...
use crate::{generate_checksum, get_len_of_lfn, get_lfn_index, is_illegal, sfn_or_lfn};
...
impl<'a> Dir<'a> {
    ...
    fn find_lfn(&self, iter: &mut DirIter, value: &str) -> Option<Entry> {
        let count = get_len_of_lfn(value);
    ...
    fn add_dot_item(&self, cluster: u32) {
        ...
        value[1] = b'.';
        di = Entry::new_sfn_bytes(self.detail.first_cluster(), &value, OpType::Dir);
...
impl<'a> DirIter<'a> {
    ...
    fn offset_value(&self) -> usize {
        self.offset + self.sector_offset * BLOCK_SIZE
    }
    ...
    fn offset_index(&mut self) {
        ...
        if self.sector_offset % spc == 0 && self.sector_offset != 0 {
